...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Replace with your Vercel production domain once deployed
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
# 2. Initialize App
app = FastAPI(title="EDKT Backend API")

# 3. Configure CORS (Allows Next.js frontend calls)
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
# 4. Initialize AI Brain Model Architecture
brain = EDKTTransformer(num_skills=50)
brain.eval()

# 5. Background Task Retraining Handler
...
# 6. Automatic Admin Seeding on Startup
...
def login_user(login_data: dict, db: Session = Depends(get_db)):
    # Support both 'matric' and 'matric_no' keys from frontend
...
def setup_db(db: Session = Depends(get_db)):
    ...
    topic1 = models.Syllabus(topic_name="Matrices")
    topic2 = models.Syllabus(topic_name="Set Theory")
    db.add_all([topic1, topic2])
    db.commit()
    
    q1 = models.Question(
        topic_id=topic1.id,
        content="What is the identity matrix of a 2x2 system?",
        option_a="[1 0, 0 1]",
        option_b="[0 1, 1 0]",
        option_c="[1 1, 1 1]",
        option_d="[0 0, 0 0]",
    ...
    q2 = models.Question(
        topic_id=topic1.id,
        content="If Matrix A is of order 2x3 and B is 3x4, what is the order of matrix AB?",
        option_a="2x3",
        option_b="3x3",
        option_c="2x4",
        option_d="4x2",
    ...
    q3 = models.Question(
        topic_id=topic2.id,
        content="If Set A = {1, 2, 3} and Set B = {3, 4, 5}, what is A ∩ B?",
        option_a="{1, 2, 3, 4, 5}",
        option_b="{3}",
        option_c="{}",
        option_d="{1, 2, 4, 5}",
    ...
    db.add_all([q1, q2, q3])
    db.commit()
    return {"message": "MAT101 Data Seeded with unique baseline questions!"}

@app.post("/setup/bulk-upload")
def bulk_upload_questions(db: Session = Depends(get_db)):
    file_path = "questions_pool.json"
    
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail="Source data file questions_pool.json not found")
    ...
    try:
        ...
        for item in questions_data:
            topic = db.query(models.Syllabus).filter(models.Syllabus.topic_name == item["topic_name"]).first()
            if not topic:
                topic = models.Syllabus(topic_name=item["topic_name"])
                db.add(topic)
                db.commit()
                db.refresh(topic)
                
            duplicate_check = db.query(models.Question).filter(models.Question.content == item["content"]).first()
            if duplicate_check:
                continue
        ...
        db.commit()
        return {"status": "success", "message": f"Ingested {added_count} evaluation elements into edkt.db"}
...
def get_adaptive_question(matric: str = None, exclude: str = "", db: Session = Depends(get_db)):
    # 1. Fetch total question pool
    all_questions = db.query(models.Question).all()
    if not all_questions:
        raise HTTPException(
            status_code=400, 
            detail="Question pool empty. Please load questions_pool.json first."
        )

    # 2. Parse exclude list sent from active frontend session (comma-separated IDs e.g. "1,4,7")
    excluded_ids = set()
    if exclude.strip():
        try:
    ...
    # 6. EDKT Model Adaptive Target Selection
    try:
        num_skills = 50
        ...
        if last_pred < 0.60 and len(logs) > 0:
            ...
            if last_failed_q:
                # Prioritize unseen question in the weak topic
...
def submit_interaction(data: schemas.InteractionSubmit, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    # 1. Clean and normalize matric string input
    clean_matric = data.matric.strip().upper() if data.matric else ""
    
    user = db.query(models.User).filter(models.User.matric_no == clean_matric).first()
    if not user:
        # If user isn't found, raise 404 instead of letting auth middleware crash with 401
...
def load_local_pool_file(db: Session = Depends(get_db)):
    ...
    try:
        # Clear existing questions and syllabus topics to avoid duplicate skips
        db.query(models.Question).delete()
        db.query(models.Syllabus).delete()
        db.commit()
